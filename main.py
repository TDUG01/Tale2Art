...
if __name__ == "__main__":
    ...
    lists = sent_tokenize(ntext)
    for i in lists:
        print(repr(i))
        
        AAinputs = tokenizer(i, return_tensors="pt", padding=True, truncation=True)
        outputs = model.generate(**AAinputs)
        print(tokenizer.decode(outputs[0], skip_special_tokens=True))
        print(GoogleTranslator(input='auto', output='en').translate(i))
